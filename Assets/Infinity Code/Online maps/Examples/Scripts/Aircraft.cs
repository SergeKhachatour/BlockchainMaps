...
namespace InfinityCode.OnlineMapsDemos
{
    ...
    public class Aircraft : MonoBehaviour
    {
        ...
        public float minSpeed = 100; // Minimum speed in km/h
        public float maxSpeed = 2000; // Maximum speed in km/h
        public float speedChangeRate = 100; // Speed change in km/h per second
        ...
        private void Update()
        {
            ...
            if (OnlineMapsInput.GetKey(KeyCode.Space))
            {
                speed = 0;
            }
            else if (OnlineMapsInput.GetKey(KeyCode.UpArrow))
            {
                speed = Mathf.Min(speed + speedChangeRate * Time.deltaTime, maxSpeed);
            }
            else if (OnlineMapsInput.GetKey(KeyCode.DownArrow))
            {
                speed = Mathf.Max(speed - speedChangeRate * Time.deltaTime, minSpeed);
            }

            if (OnlineMapsInput.GetKey(KeyCode.LeftArrow) || OnlineMapsInput.GetKey(KeyCode.A))
            {
                tilt -= Time.deltaTime * tiltSpeed * maxTilt;
            }
            else if (OnlineMapsInput.GetKey(KeyCode.RightArrow) || OnlineMapsInput.GetKey(KeyCode.D))
            {
                tilt += Time.deltaTime * tiltSpeed * maxTilt;
            }
            else if (tilt != 0)
            ...
            if (OnlineMapsInput.GetKey(KeyCode.W))
            {
                altitude += altitudeChangeSpeed * Time.deltaTime;
            }
            else if (OnlineMapsInput.GetKey(KeyCode.S))
            {
                altitude -= altitudeChangeSpeed * Time.deltaTime;
            }
